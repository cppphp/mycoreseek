...
public:
	...
	u1 only_use_kwdict; //update by qifei
	Segmenter_ConfigObj():
		merge_number_and_ascii(0),
		seperate_number_ascii(0),
		compress_space(0),
		only_use_kwdict(0) //update by qifei
	{
...
class Segmenter {
 ...
 public:
	...
	const u1* peekAndBuildToken(u2& aLen, u2& aSymLen, u2 n = 0);
	const u1* peekAndBuildKwToken(u2& aLen, u2& aSymLen); //qifei
