...
#define MAX_TOKEN_LENGTH 36 //update by qifei
...
{
	if (m_config && m_config->only_use_kwdict && m_kwdict) {
		return peekAndBuildKwToken(aLen, aSymLen);
	} else {
		return peekAndBuildToken(aLen, aSymLen, n);
	}
}

const u1* Segmenter::peekAndBuildToken(u2& aLen, u2& aSymLen, u2 n)
{
	//check is sep char
	//skip \r
	//reset unigram when \n
	...
	while(*ptr && (ptr<m_buffer_end) && i<CHUNK_BUFFER_SIZE){
		...
		if (!m_config || !m_config->only_use_kwdict) { //让中英符号可以在一起成词
...
const u1* Segmenter::peekAndBuildKwToken(u2& aLen, u2& aSymLen) //qifei
{
	if (m_thunk.maxLength() > -1) {
		u1* tok;
		u2 pos = 0;
		m_thunk.peekKwToken(pos, aLen);
		aSymLen = aLen;
		tok = &m_buffer_chunk_begin[pos];
		//检查同义词
		if (aLen) {
			tok = m_buffer_ptr;
			int sym_key_len = 64;
	...
	while(*ptr && (ptr<m_buffer_end) && i<CHUNK_BUFFER_SIZE){
		...
		//check tagger
		int num = m_kwdict->findHits((const char*)ptr,&rs[1],1024-1, MAX_TOKEN_LENGTH);
		m_thunk.setKwItems(i, num, &rs[1]); //set to kword
		ptr +=  len;
		i+=len;
	}
	if (m_thunk.maxLength() > 0)
		return peekKwToken(aLen, aSymLen);
	return NULL;
}
