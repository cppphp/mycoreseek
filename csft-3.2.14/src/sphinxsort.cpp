...
protected:
	static inline int GetSegment ( SphAttr_t iStamp, SphAttr_t iNow )
	{
		...
		if ( iStamp>=iNow-86400 ) return 1; // last day
		if ( iStamp>=iNow-604800 ) return 2; // last week
		if ( iStamp>=iNow-2592000 ) return 3; // last month
		if ( iStamp>=iNow-7776000 ) return 4; // last 3 months
		if ( iStamp>=iNow-93312000 ) return (iNow-iStamp)/(15552000) + 5; // last 36 months
		return 12; //qifei
	}
};
